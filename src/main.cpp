...
{   
    ...
    // CSRMatrix L = CSRMatrix(MATRIX_DIM, 5*DIM_X*DIM_X*DIM_Y); // store the L matrix
    // CSRMatrix Lt = CSRMatrix(MATRIX_DIM, 5*DIM_X*DIM_X*DIM_Y); // store the L matrix's transpose
    // double *D = (double *) malloc(dim*dim*sizeof(double)); // store the diagonal matrix
